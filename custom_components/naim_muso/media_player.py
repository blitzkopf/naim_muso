...
from .base_entity import BaseEntity, ensure_device
...
async def async_setup_entry(
    ...
    entity = NaimMediaPlayer(coordinator=entry.runtime_data.coordinator, parameter=None)
...
class NaimMediaPlayer(BaseEntity, MediaPlayerEntity):
    ...
    def _device(self) -> NaimCo | None:
        return getattr(self.coordinator, "_device", None)
    ...
    def state(self) -> MediaPlayerState | None:
        ...
        stbystate = None
        assert self._device and self._device.state and self._device.standbystatus
        try:
        ...
        if (
            self._device.state.viewstate
            and self._device.state.viewstate.get("phase") == "PAUSE"
        ):
            return MediaPlayerState.PAUSED
        if (
            self._device.state.viewstate
            and self._device.state.viewstate.get("state") == "PLAYING"
        ):
    ...
    def source_list(self) -> list[str] | None:
        ...
        return [str(input_value) for input_value in inputs.values()]

    @ensure_device
    async def async_select_source(self, source: str) -> None:
        """Select input source."""
        _LOGGER.error("Selecting source: %s", source)
        inputs = self._device.inputs
    ...
    async def async_browse_media(
        ...
        return await media_browser.async_browse_media(
            self._device, media_content_type, media_content_id
        )
    ...
    async def async_play_media(
        ...
        **kwargs: Any,
        ...
        if not self._device or not self.available:
            return
        return await media_browser.async_play_media(
            self._device, media_type, media_id, enqueue, announce, **kwargs
        )
